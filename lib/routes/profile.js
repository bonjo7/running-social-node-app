...
router.get("/myprofile", auth, async (req, res) => {
  try {
    ...
    if (!profile) {
      console.log("1 No profile available for the selected user");
...
router.get("/user/:user_id", async (req, res) => {
  try {
    ...
    if (!profileID) {
      console.log("2 No profile available for the selected user");
      return res
        .status(400)
    ...
    if (err.kind == "ObjectId") {
      console.log("3 No profile available for the selected user");
      return res
        .status(400)
        .json({ msg: "No profile available for the selected user" });
    } else {
