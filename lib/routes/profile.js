...
router.get('/user/:user_id', async(req, res) => {
    ...
    try {
        const profileID = await Profile.findOneAndUpdate({ user: req.params.user_id }).populate('user', ['name', 'email']);
        
        if(!profileID) {

            console.log('No profile available for the selected user');
            return res.status(400).json({ msg: 'No profile available for the selected user'});
